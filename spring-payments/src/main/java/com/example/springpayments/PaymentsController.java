...
public class PaymentsController {
    ...
    // Create a collection of the messages into an ArrayList
    class ErrorMessages{
    ...
    public String postAction(@Valid @ModelAttribute("command") PaymentsCommand command,  
        ...
        if(command.zip().equals(""))            {hasErrors = true; msgs.add("Zip Code Required.");}
        ...
        if(!command.cardnum().matches("\\d{4}-\\d{4}-\\d{4}-\\d{4}"))   {hasErrors = true; msgs.add("Invalid Credit Card Number:  " + command.cardnum());}
        if(!command.cardexpyear().matches("\\d{4}"))                    {hasErrors = true; msgs.add("Invalid Credit Card Year:  " + command.cardexpyear());}
        if(!command.cardcvv().matches("\\d{3}"))                        {hasErrors = true; msgs.add("Invalid Credit Card CVV:  " + command.cardcvv());}

        // months of the year validation
        if(months.get(command.cardexpmon()) == null)                {hasErrors = true; msgs.add("Invalid Card Expiration Month:  " + command.cardexpmon());}

        // U.S. states validation
        ...
        auth.transactionAmount = "0.05" ; // changed from 30.00 to 0.05
        ...
        boolean captureValid = true; //originally false
        CaptureRequest capture = new CaptureRequest();
        CaptureResponse response = new CaptureResponse();
        if(authValid){
            capture.reference = order_num;
            capture.paymentId = authResponse.id;
            capture.transactionAmount = "0.05"; // changed from 30.00 to 0.05
            capture.transactionCurrency = "USD";
            System.out.println("\n\nCapture Request :  " + capture.toJson());
            response = api.capture(capture);
